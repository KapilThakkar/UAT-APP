var project = {
    ...
    oldStakeholders:[],
    SetLeadId: function () {
        project.LeadName = $('#VersionLead').val() //Newly Added by Atul.s,
    },
    pageload: function () {
        /*isPortfolioOn = false;
        ...
        $('#navHeading tr:eq(1) td img').attr('src', '/images/Admin/b_Current Folder.png');
        ...
        $('#pageTab h2:eq(1)').click(function () {
            ...
            if ($("#pName")[0].tagName.toLowerCase() != "select") {
                ...
                $("#pName").after('<a href="#" class="addNew addNew1" id="addProject"><img title="Add Project" src="/images/Admin/add.png" style="width:23px;height:23px;vertical-align:top;margin-left:7px"/></a>');
    ...
    clear: function () {
        ...
        //New code added by Atul.s
        $('#VersionLead').val('');
        $('#HdnVersionLead').val('');
        $('#VersionStakeholder').val('');
        $('#VersionStakeholder').html('');
        $('#VersionLead').val('');
        // End
    ...
    btnSavePortfolio: function (stakeHolderDetailsArr) {
        project.SetLeadId();
        var saveJson = '';
        var HdnVersion = $('#HdnVersionLead').val();
        if (!project.isPortfolioOn)
        ...
        var leadAlias = HdnVersion.split('|')[0]// $('div[title="lead"] #divEntityData').attr('key');// Commented by Atul.s
        //if (leadAlias == null || leadAlias == undefined || leadAlias == '') {
        //    var msgTag = 'Version Lead cannot be empty!'//added by Mohini for resource file
        ...
        //    return;
        //}
        var orignalVLead = $('#HdnVersionOrignalTxt').val();
        if (HdnVersion == '' || (orignalVLead != project.LeadName)) {
            var msgTag = 'Please Select Proper Version Lead !'//added by Mohini for resource file
            project.alertBox(msgTag);
            $('#Projectform').show();
            Main.hideLoading();
            addPrjFlag == 1;
        ...
        */   //---- Commented by Atul.S
        ...
        if (project.stakeholder_spUserId.length > 20) {
            ...
            // var listN = jP.Lists.setSPObject(project.SiteURL, 'Project');   //---- Commented by Atul.S
            /*  Old Code Commented by Atul.s
            ...
            */
            ...
            /*-----New Code for Version Lead------*/
            var lead, SPUserID = '', leadMailId, leadVal;
            project.leadName = '';

            if (HdnVersion != '' && HdnVersion != undefined) {
                leadMailId = HdnVersion.split('|')[2];
                project.leadName = HdnVersion.split('|')[0];
                leadVal = HdnVersion.split('|')[0];
                SPUserID = HdnVersion.split('|')[1];
                if (leadVal.indexOf(";") != -1) {
                    leadVal = leadVal.replace(';', '');
                }
            }
            ...
            if ((document.getElementById('startDate').value.length == 0) || (document.getElementById('endDate').value.length == 0)) {
                ...
                if (prjName.length > 55) {
                    ...
                    if (addPrjFlag != 1) {
                        ...
                        var res = ServiceLayer.GetProjectData('ValidateProjectName', data, 'ProjectMgmt');
                        if (res != '' && res != undefined) {
                            if (res.Success.toLowerCase() != 'not exist') {
                    ...
                    if (addPrjFlag == 0) {
                        //Project Lead Info 
                        AddUser.testerSPID = '';
                        var globTestName_New = "i:0#.f|membership|" + (leadMailId != '') ? leadMailId.toString() : ""
                        AddUser.globTesterName = globTestName_New //  $('div[title="lead"] #divEntityData').attr('key');//Added for SP13 Commented by Atul.S
                        AddUser.presentInGroup = 0;
                        /* Commented by Atul.S
                        AddUser.GetGroupNameforUser();
                        if (AddUser.presentInGroup == 1)//User is not present in member group
                        ...
                        */
                        // SPUserID =AddUser.testerSPID;  Commented by Atul.S
                        ...
                        ///if (false)
                        {
                            ...
                            */
                            ...
                            /* New Code Added by Atul.s 
                            var ele = $('#HdnVersionStakeHolder').val();
                            if (ele != '') {

                                for (var i = 0; i < ele.split(';').length; i++) {
                                    var val = ele.split(';')[i].split('|');
                                    listProjectUsers.push({
                                        "userName": val[0],
                                        "email": val[2],
                                        "spUserId": val[1],
                                        "securityId": "5",
                                        "alias": "i:0#.f|membership|" + val[2].toString()
                                    });
                                }
                            }
                            /*End*/
                            ...
                            /*for Stakeholder Remove ##change*/
                            var ele = $('#VersionStakeholder span');
                            $('#VersionStakeholder span').each(function (ind, item) {

                                var val = $(item).attr('val').split('|');
                                listProjectUsers.push({
                                    "userName": val[0],
                                    "email": val[2],
                                    "spUserId": val[1],
                            ...
                            var retPrjId = ServiceLayer.InsertUpdateData('InsertUpdateProjectDetails', NewJsonIns, 'ProjectMgmt');

                            if (retPrjId != "" && retPrjId != undefined) {
                                if (retPrjId.Success == "Done") {
                                    ...
                                    if (SPUserID == _spUserId || $.inArray(_spUserId.toString(), project.stakeholder_spUserId) != -1 || $.inArray('1', security.userType) != -1) {
                                        show.projectId = retPrjId.Value;

                                        /////Added by Mangesh for addition of latest user association data for stakeholder	
                                        security.userAssociationForProject[retPrjId.Value] = new Array();
                                        if (SPUserID == _spUserId) {
                                            security.userAssociationForProject[retPrjId.Value].push('2');
                                        }
                                        if ($.inArray(_spUserId.toString(), project.stakeholder_spUserId) != -1) {
                                            security.userAssociationForProject[retPrjId.Value].push('5');
                                        }
                                        //////////////////////////////////////////////////////
                                    }
                                    else {
    ...
    compressFunPortfolio: function () {
        ...
        Main.showLoading();
        //show.showData();
        //AddUser.GetGroups(); /*comment by Atul*/
    ...
    clearPortfolio: function () {
        $("div[title='lead']").html('');
        $("textarea[title='lead']").text('');
        /*for Bug 13813 disable/Initially Enable textbox when Tester try to Update Version Lead*/
        $('#VersionLead').attr('disabled', false);
        /*end*/
        $("div[title='stakeholders']").html('');
        $("textarea[title='stakeholders']").text('');
    ...
    AttachDropDownDelegates: function () {
        $("#group").change(function () {
            ...
            for (var i = 0; i < len ; i++) {
                var portfolio = project.gGroupPortfolioArr[$("#group option:selected").text()] != undefined ? project.gGroupPortfolioArr[$("#group option:selected").text()][i] : "";
                var portfolioId = project.gGroupPortfolioIdArr[$("#group option:selected").text()] != undefined ? project.gGroupPortfolioIdArr[$("#group option:selected").text()][i] : "";
    ...
    cancelPortfolio: function (i) {
        ...
        if (project.oldLead == null || project.oldLead == undefined)
            project.oldLead = '';
        //New code Added by Atul.S
        var VersionLead = $('#VersionLead');
        var HdnVersionLead = $('#HdnVersionLead');
        $('#HdnVersionOrignalTxt').val(project.oldLead);
        VersionLead.val(project.oldLead);
        HdnVal = project.oldLead + "|" + project.oldLeadSPUserId + "|" + project.oldLeadMailId;
        HdnVersionLead.val(HdnVal);


        var span = ''; var SpanArray = [];
       
        project.oldStakeholders.join(';');
        if (project.oldStakeholders.length > 0) {
            for (var sti = 0; sti < project.oldStakeholders.length; sti++) {
                span += project.oldStakeholders[sti]
            }
        }
        $('#VersionStakeholder').html(span);
        //END
    ...
    updatePortfolio: function () {
        ...
        //    return;
        //}
        //AddUser.globTesterName = "i:0#.f|membership|" + (HdnVersion.split('|')[2] == '' ? '' : HdnVersion.split('|')[2].toString())    // $('div[title="lead"] #divEntityData').attr('key');//Added for SP13
        ...
        */
        var SPUserID = HdnVersion.split('|')[1]//AddUser.testerSPID;
        var leadAlias = AddUser.globTesterName;
        // var lead = $("textarea[Title='lead']").val();
        var leadMailId = HdnVersion.split('|')[2]  //project.resolveTesterEmail(lead).join();
        var leadName = HdnVersion.split('|')[0]// project.resolveTesterFullName(lead).join();
    ...
    editModePortfolio: function (i) {
        ...
        if (security.userType.length != 0) {
            if ($.inArray('1', security.userType) != -1) {
                ...
                if (($.inArray('5', userAsso) != -1)) {
                    $("a[title='Browse']").eq(0).show();
                    /*for Bug 13813 disable textbox when Tester try to Update Version Lead*/
                    $('#VersionLead').attr('disabled', false);
                    /*end*/
                    document.getElementById('btnUpdate').style.display = "inline";
                    document.getElementById('btnCancel').style.display = "inline";
                    ...
                    /*for Bug 13813 disable textbox when Tester try to Update Version Lead*/
                    $('#VersionLead').attr('disabled', true);
                    /*end*/
        ...
        if (lstSL != '' && lstSL != null && lstSL.length > 0) {
            ...
            var span = ''; var SpanArray = []; var NewArrayforRemoveStakeholder = [];
            /*End*/
            for (sl = 0; sl < lstSL.length; sl++) {
                if (lstSL[sl].securityId == '2') {
                    ...
                    var ele = '';
                    ele = lstSL[sl].userName + '|' + lstSL[sl].spUserId + '|' + lstSL[sl].email
                    eles.push(ele)

                    NewArrayforRemoveStakeholder.push(
                                  {
                                      'name': lstSL[sl].userName,
                                      'value': ele
                                  })
            ...
            $('#VersionStakeholder').val('');
            $('#VersionStakeholder').html('');
            $('#HdnVersionStakeHolder').val('');

            /*New code Added by Atul.s*/
            /*for Stakeholder Remove ##change*/
            for (var k = 0; k < NewArrayforRemoveStakeholder.length; k++) {
                SpanArray.push('<span contenteditable="true" val="' + NewArrayforRemoveStakeholder[k].value + '" class="all-copy">' + NewArrayforRemoveStakeholder[k].name + ';' + '</span>')
            }
            project.oldStakeholders = SpanArray;
            SpanArray.join(';');
            if (SpanArray.length > 0) {
                for (var sti = 0; sti < SpanArray.length; sti++) {
                    span += SpanArray[sti]
                }
            }
            $('#VersionStakeholder').html(span);
            ...
            // $('#VersionStakeholder').val(stakeHolderNames);
            $('#HdnVersionStakeHolder').val(eles.join(';'));
        ...
        for (var i = 0; i < project.ProjectListItems.length; i++) {
            if (project.ProjectListItems[i].projectId == projectId) {
                if (project.ProjectListItems[i].testpass_Count != "0" && project.ProjectListItems[i].testpass_Count != undefined && project.ProjectListItems[i].testpass_Count != "") {
                    ...
                    var res = ServiceLayer.DeleteData('DeleteProject', data, 'ProjectMgmt');
                    if (res != "" && res != undefined) {
                        if (res.Success == "Done") {
                            ...
                            project.alertBox('Sorry! Something went wrong, please try again!'); Main.hideLoading();
                    }
                    else
                        project.alertBox('Sorry! Something went wrong, please try again!'); Main.hideLoading();
                }
            ...
            function SaveDetails() {
                ...
                if (!isNameExist) {
                    ...
                    if (choice == "group") {
                        ...
                        var resGpI = ServiceLayer.InsertUpdateData('InsertGroup', objInsertGroup, 'ProjectMgmt');
                        if (resGpI != '' && resGpI != undefined && resGpI != null) {
                            if (resGpI.Success == "Done") {
                                $("#group").append('<option value="' + resGpI.Value + '" title="' + entityValue + '">' + entityValue + '</option>');
                                $("#group").val(resGpI.Value);
                                $("#group").change();
                            }
                        }
                    }
                    if (choice == "process") {
                        ...
                        var resPoI = ServiceLayer.InsertUpdateData('InsertPortfolio', objInsertProcess, 'ProjectMgmt');

                        if (resPoI != '' && resPoI != undefined && resPoI != null) {
                            if (resPoI.Success == "Done") {
                                $("#process").append('<option value="' + resPoI.Value + '" title="' + entityValue + '">' + entityValue + '</option>');
                                $("#process").val(resPoI.Value);
                                $("#process").change();
                            }
                        }
                        ...
                        if (project.gGroupPortfolioArr[$("#group option:selected").text()] == undefined) {
                            ...
                            project.gGroupPortfolioIdArr[$("#group option:selected").text()].push(resPoI.Value);
                        }
                        else {
    ...
    delGroupOK: function () {
        ...
        if (portfolios == null || portfolios == undefined || portfolios.length == 0) {
            ...
            var res = ServiceLayer.DeleteData('DeleteGroup', groupValue, 'ProjectMgmt');
    ...
    delPortfolioOK: function () {
        ...
        if (projects == null || projects == undefined || projects.length == 0) {
            ...
            var res = ServiceLayer.DeleteData('DeletePortfolio', portfolioValue, 'ProjectMgmt');
    ...
    getGPPArray: function () {/*PNew fn*/

        var resultCollection = new Array();
        var TempArr = ServiceLayer.GetData('GetGrpPortProjects', undefined, 'ProjectMgmt');
        if (TempArr != undefined && TempArr.length > 0) {
            var grpId = []; var port;
            var Len = TempArr.length;
            for (var i = 0; i < Len; i++) {
                if (TempArr[i].hasOwnProperty("Group_Id")) {
                    if ($.inArray(TempArr[i].Group_Id, grpId) == -1) {
                        grpId.push(TempArr[i].Group_Id);
                    }
                }
            }
            ...
            if (grpId != undefined && grpId.length > 0) {
                var valuesArray = [];
                for (var k = 0; k < grpId.length; k++) {
                    var listPortFolio = [];
                    for (var j = 0; j < Len; j++) {
                        if (parseInt(grpId[k]) == parseInt(TempArr[j].Group_Id)) {
                            port = [];
                            port.push(TempArr[j]);

                            if (port.length > 0) {
                                for (var l = 0; l < port.length; l++) {

                                    var Portfolio = {}



                                    Portfolio['portfolioId'] = port[l].Portfolio_Id.toString();
                                    Portfolio['portfolioName'] = port[l].Portfolio_Name.toString();
                                    listPortFolio.push(Portfolio);

                                    resultCollection[k] = [];
                                    resultCollection[k].groupId = port[l].Group_Id.toString();
                                    resultCollection[k].groupName = port[l].Group_Name.toString();
                                    resultCollection[k].listPortfolio = listPortFolio;
                                }
            ...
            /*
             if (dtGroupPortData != null && dtGroupPortData.Rows.Count > 0)
                {
                    var groups = (from DataRow dr in dtGroupPortData.Rows
                                  select dr["Group_Id"]).Distinct();

                    foreach (int gId in groups)
                    {
                        GroupPortfolio GrPt = new GroupPortfolio();
                        List<Portfolio> listPortFolio = new List<Portfolio>();

                        DataTable port = (from DataRow drport in dtGroupPortData.Rows
                                          where drport["Group_Id"].ToString() == Convert.ToString(gId)
                                          select drport).CopyToDataTable();

                        foreach (DataRow drpId in port.Rows)
                        {
                            if (string.IsNullOrEmpty(GrPt.groupId))
                                GrPt.groupId = (drpId.IsNull("Group_Id")) == true ? "" : Convert.ToString(drpId["Group_Id"]);
                            if (string.IsNullOrEmpty(GrPt.groupName))
                                GrPt.groupName = (drpId.IsNull("Group_Name")) == true ? "" : Convert.ToString(drpId["Group_Name"]);

                            listPortFolio.Add(new Portfolio()
                            {
                                portfolioId = (drpId.IsNull("Portfolio_Id")) == true ? "" : Convert.ToString(drpId["Portfolio_Id"]),
                                portfolioName = (drpId.IsNull("Portfolio_Name")) == true ? "" : Convert.ToString(drpId["Portfolio_Name"])
                            });
                        }
            */
    ...
    getProjectList: function () {/*PNew fn*/
        Main.showLoading();

        project.ProjectListItems = ServiceLayer.GetData('GetProjectDetails', undefined, 'ProjectMgmt');
    ...
    getMarkup: function () {/*PNew fn*/
        ...
        if (parr != null && parr != undefined) {
            for (i4 = 0; i4 < parr.length; i4++) {
                ...
                '<img style="width:25px;height:25px;padding-right:7px" src="/images/Admin/Edit1.png"></a>' +
                '<a onclick="project.delPortfolio(' + parr[i4].projectId + ')" style="cursor:pointer;" title="Delete ' + project.gConfigProject + '/' + project.gConfigVersion + '" ><img style="width: 25px; height: 25px" src="/images/Admin/Garbage1.png"></a></td></tr>';
